% 绘制N-BV关系图 - 正确实现对数坐标轴
% 确保Y轴使用正确的对数刻度（logarithmic scale）
...
% 设置对数坐标轴的刻度
set(ax, 'YTick', [0.1, 1, 10, 100, 1000]);
set(ax, 'YTickLabel', {'0.1', '1', '10', '100', '1000'});
...
%% 验证对数坐标轴
fprintf('\n对数坐标轴验证:\n');
fprintf('Y轴刻度: [0.1, 1, 10, 100, 1000]\n');
fprintf('数据点范围: N_min=%.4f, N_max=%.4f\n', min(min(N_matlab), min(N_medici)), max(max(N_matlab), max(N_medici)));
